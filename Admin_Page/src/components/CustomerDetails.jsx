...
const Details = () => {
  ...
  // Function to format appointment date for display
  ...
  // Function to filter appointments based on type (today, upcoming, other)
    ...
    if (filterType === "today") {
      ...
      return appointments
        ...
        .filter(
          (appointment) =>
            new Date(appointment.date).toDateString() !== today.toDateString()
        ) // Exclude appointments that are today
  ...
  // Function to render the appointments table based on filter type
      ...
      <table width="1450" className="table1 mt-3">
        ...
        <tbody>
          {appointments.map((appointment, index) => (
            <tr key={appointment.id} className={index % 2 === 0 ? "even" : "odd"}>
              <td>
  ...
  // If still loading appointments, display a loading message
  if (loading) {
    return <div>Loading...</div>;
  }

  // If there's an error fetching appointments, display the error message
  if (error) {
    return <div>{error}</div>;
  }

  // Render the component with the header and filtered appointment tables
  return (
    <>
      <AdminHeader />
      {renderTable(filterAppointments("today"), "Today's Appointments", "today")}
      {renderTable(filterAppointments("upcoming"), "Upcoming Appointments", "upcoming")}
